...
> **Caveat resolved — balanced control confirms the null.** The DNA SAE was trained **unbalanced** (98% DNA / 2% text), so a skeptic could argue text/cross-modal features were merely *under-trained*. We trained a **balanced DNA SAE** (70% DNA / 30% text — text upweighted 15×) and re-ran φ (9,559 samples): **0 features with φ>0.3** (vs 2 structural ones unbalanced), top φ=0.07. Text features formed abundantly (1,294 co-fire on both bands; many fire on 7k–9k text samples), so text was *not* starved — there simply are no selectively co-firing cross-modal features. The null is **real, not a training-imbalance artifact.** (The balanced SAE has 54.8% dead latents from the small ~10M-token balanced set — but that's unused DNA-side capacity, not missing text features, so it doesn't undercut the conclusion.)
