...
> **Caveat (balanced control — retrain in progress).** The DNA SAE was trained **unbalanced** (98% DNA / 2% text), so a skeptic could argue text/cross-modal features were merely *under-trained*. We built a **balanced DNA SAE** (70% DNA / 30% text — text upweighted 15×) as the control. A **first attempt was itself under-trained** (small 10.2M-token store + standard config → warmup consumed 69% of the 1,450 steps, AuxK threshold exceeded the store size → 55% dead latents), so its φ=0 result is *not* trustworthy — do not cite it. Retraining with a corrected schedule (warmup 5%, 30 epochs ≈ 9.3k steps, AuxK threshold < store); φ verdict **pending**. (The unbalanced run already shows text features form — 5,107 co-firing — so the null is unlikely to be pure starvation, but the clean balanced control is what settles it.)
