...
> **Balanced control — done properly, confirms the null.** The DNA SAE was trained **unbalanced** (98% DNA / 2% text), so a skeptic could argue cross-modal features were merely *under-trained*. We built a **balanced DNA SAE** (70% DNA / 30% text — text upweighted 15×; properly trained: warmup 5%, 30 epochs, 25.6% dead — an earlier under-trained attempt was discarded). φ on 9,559 samples: **1 selective feature at φ>0.3 — F28983 (φ=0.572, r=0.703).** But characterizing it: its **text-side fires only at token_index 4105–4145 (the DNA-END→question-onset boundary), and its DNA-side fires distributed across the sequence (not variant-specific)** — the same structural signature as the unbalanced F15966. So the one high-φ feature is a **boundary/transition marker, not semantic binding.** Even with text upweighted 15× and a properly trained SAE, there is **no genuine semantic cross-modal feature** — the null is real, not a training-imbalance artifact.
