...
- **L32 vs L30 (dose-controlled):** L32 synapse cluster re-steered with the SAME p95 per-feature dosing that makes L30 work (`--scale-file load_bearing_l32.npz --scale-key ma_text`, per-feature scales 9.8/3.3/4.9/6.3/7.0/7.9, α=0,4,6,8, decision-point steer-after-25). Result: **0/32 generations contain any synapse content** (exact-word AND semantic scan = 0%), coherence stays 100% (distinct3≈1.0) — no effect, not salad. So **L32 genuine synapse injection = 0% vs L30's 67%**, now dose-controlled: L30 is the steering sweet spot on its own merits, not because L32 was under-dosed. Jared's 'deeper=more steerable' does **not** hold here. (D9_L32synapse_p95.jsonl)
