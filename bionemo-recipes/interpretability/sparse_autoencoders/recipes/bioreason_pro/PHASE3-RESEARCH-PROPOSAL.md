...
> **Figure 3.** Where the tokens land, raw vs SAE (same protein/text tokens, UMAP). The raw residual embedding (a) already separates the modalities; the SAE codes inherit that separation whether the loss is unbalanced (b) or balanced (c). Balancing changes *which features carry protein* (Figure 2), not the token-level layout — the modality split is a property of the encoder, which the SAE preserves.
...
> **Figure 4.** Layer-16 activations (UMAP). Protein (ESM3) and text form near-separate clusters (left); DNA (Evo 2) and text overlap more (right) — which is why a raw-activation cross-modal metric reads near zero for an encoder that was never aligned to text.
