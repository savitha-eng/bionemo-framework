...
**So far I haven't found any cross-modal (fusion) features — the most interesting result.** Searching for features that bind a biological concept to its text turns up only structural markers, in protein or DNA, balanced or not. As for *why*: the cross-modal metric (from SAE-V) compares the model's raw activations, and in BioReason-Pro the protein and text activations point in almost entirely separate directions (Figure 4; the DNA model overlaps somewhat more), which the SAE inherits regardless of balancing (Figure 3). One hypothesis — to test, not a conclusion — is that feature-level fusion needs the two modalities to be aligned: the vision–language models where fusion shows up use encoders trained against text (CLIP), whereas ESM3 and Evo 2 never saw text. I don't yet know whether alignment is the deciding factor, and testing it directly is the main next step.
