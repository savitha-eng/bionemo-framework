...
- **Reliability-first, enrichment-grounded protein labels.** Raw amino-acid windows are uninterpretable by an LLM ("fires on leucine"). Instead, for the proteins a feature fires on we run a **Fisher over-representation test** against GO annotations and UniProt keywords/domains — a *statistical* label with a p-value, no hallucination. Examples: **F9422 = mitochondrial-ribosome detector** (GO rRNA-binding + UniProt *Transit peptide* p=4.9e-9); **F33529 = protein-kinase detector** (UniProt *Protein kinase* domain, p=3.6e-14).
