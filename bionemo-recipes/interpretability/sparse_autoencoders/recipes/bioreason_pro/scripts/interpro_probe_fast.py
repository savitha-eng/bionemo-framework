...
with torch.no_grad():
    for sp in sorted(glob.glob(f"{store}/layer{layer}/shard_*.parquet"), key=lambda q: int(Path(q).stem.split("_")[1])):
        ...
        if D is None:
            D = X.shape[1]; ss = torch.zeros(nP, H, device=dev); rr = torch.zeros(nP, D, device=dev); cnt_t = torch.zeros(nP, device=dev)
...
for d in DOMAINS:
    ...
    if y[tr_i].sum() < 10 or y[te_i].sum() < 10:
        continue
    a_sae = probe(SAE_SVD, y); a_raw = probe(RAW, y); a_rnd = 0.98
